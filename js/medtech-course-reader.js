...
function lessonHtml(l,idx,total){
 ...
 if(self.length)html+='<details><summary><strong>Self-check</strong></summary>'+self.map((q,qi)=>{
   ...
   return '<div class="mt-callout"><strong>'+esc(q.question)+'</strong>'+(q.answer_options||[]).map((o,oi)=>'<button class="mt-choice" data-self="'+key+'" data-opt="'+oi+'" aria-pressed="'+(String(chosen)===String(oi))+'">'+esc(o)+'</button>').join('')+'<p class="mt-meta">Preview choice only. Correct-answer scoring is reserved for the secure released assessment.</p></div>';
 }).join('')+'</details>';
